...
function PostCardComponent({ post, profile, onToggleSaved, onEdit, onDelete }: PostCardProps) {
  ...
  return (
    ...
    <Card variant="default" padding="md" className={`relative transition-colors font-sans ${post.pinned ? 'border-sky shadow-sm bg-sky/5' : ''}`}>
       ...
       <div className="flex-1 flex justify-between items-start">
         <div>
           <h3 className="font-bold text-lg text-slate">
             {post.authorName || 'Usuário'} <span className="text-xs font-normal text-muted">• {post.authorRole === 'MEMBRO_ATIVO' ? 'Membro Ativo' : post.authorRole === 'MEMBRO_APOSENTADO' ? 'Membro Aposentado' : 'Administrador'}</span>
           </h3>
           <div className="flex items-center">
             <p className="text-xs uppercase text-muted font-bold tracking-wider mt-0.5">#{post.category}</p>
           </div>
         </div>
         <div className="flex items-center gap-1">
           ...
           <Button
             ...
             className={`min-h-[44px] min-w-[44px] ${isSaved ? 'text-sky bg-sky/10' : 'text-muted hover:text-navy hover:bg-ice'}`}
     ...
     <div
       className="text-sm leading-relaxed mb-6 text-slate line-clamp-3 prose prose-slate prose-sm text-pretty"
     ...
     <div className="flex gap-4 border-t border-border-gray/50 pt-4">
       ...
       <Link to={`/feed/${post.id}#comment`} className="text-xs font-bold text-muted flex items-center gap-2 hover:text-navy transition-colors focus:ring-2 focus:ring-navy focus:outline-none min-h-[44px]">
