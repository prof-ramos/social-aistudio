...
export function PostCard({ post, profile, onToggleSaved }: PostCardProps) {
  ...
  return (
    <div className={`bg-white border p-6 relative transition-colors font-sans ${post.pinned ? 'border-sky shadow-sm bg-sky/5' : 'border-border-gray'}`}>
       {post.pinned && (
     ...
     <div className="flex gap-4 mb-4">
       ...
       <div className="flex-1 flex justify-between items-start">
         ...
         <button 
           ...
           className={`p-2 transition-colors ${isSaved ? 'text-sky bg-sky/10' : 'text-slate/30 hover:text-navy hover:bg-ice'} rounded-none`}
           title="Salvar Post"
         >
