    ...
    <PageContainer variant="feed" className="flex h-full flex-col space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate text-base font-medium leading-relaxed">Fichas e relatos alimentados pelos Oficiais de Chancelaria</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-max pb-8">
        {filtered.map(posto => {
          const flagCode = getFlagCode(posto.country);
          return (
            <Link key={posto.id} to={`/postos/${posto.slug}`} className="block group">
              <Card variant="elevated" padding="md" className="h-full transition-transform hover:-translate-y-1 hover:shadow-lg duration-200 flex flex-col">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-12 h-12 bg-ice border border-border-gray/50 flex items-center justify-center text-navy group-hover:bg-navy group-hover:text-white transition-colors overflow-hidden">
                    {flagCode ? (
                      <span className={`fi fi-${flagCode} text-3xl`} aria-hidden="true" />
                    ) : (
      ...
      {filtered.length === 0 && (
          <Card variant="default" padding="none" className="text-center py-16 text-slate">
            <MapPin className="w-12 h-12 mx-auto mb-4 text-slate" />
            <p className="font-serif text-xl text-navy mb-1 leading-relaxed">Nenhum posto encontrado</p>
            <p className="text-base text-slate font-medium leading-relaxed">Não encontramos resultados para "{search}". Tente buscar por outro termo.</p>
         </Card>
      )}
    </PageContainer>
  );
